{ stdenv, lib, fetchzip, autoPatchelfHook, jdk23, glib, gtk3, xorg, ffmpeg_6-full, sdrplay, glibc, libX11, makeWrapper}:
let
    version = "0.6.1";
in
stdenv.mkDerivation {
    ...
    installPhase = ''
        ...
        # Set environment variables to ensure GTK3 libraries are found
        mkdir -p $out/etc/profile.d
        echo 'export GTK_PATH='$out/lib/gtk-3.0 >> $out/etc/profile.d/gtk3.sh
        echo 'export LD_LIBRARY_PATH='$out/lib:$out/lib/gtk-3.0:$LD_LIBRARY_PATH >> $out/etc/profile.d/gtk3.sh
